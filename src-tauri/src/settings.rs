...
pub fn load_settings() -> AppSettings {
    ...
    let mut file = match File::open(&path) {
        Ok(f) => f,
        Err(error) => {
            eprintln!("Failed to read settings '{}': {}; using defaults", path.display(), error);
            return AppSettings::default();
        }
    };
    ...
    toml::from_str(&contents).unwrap_or_else(|error| {
        eprintln!("Failed to parse settings '{}': {}; using defaults", path.display(), error);
        AppSettings::default()
    })
}
...
pub fn save_settings(settings: &AppSettings) -> Result<(), String> {
    ...
    if let Some(parent) = path.parent() {
        create_dir_all(parent).map_err(|e| {
            format!("Failed to create settings directory '{}': {}", parent.display(), e)
        })?;
    }
    ...
    file.write_all(toml_string.as_bytes())
        .map_err(|e| format!("Failed to write settings '{}': {}", path.display(), e))?;
